...
# Machine Learning Model Pages
    ...
    # Filter data based on selected input
    filtered_data = death_data[(death_data["import streamlit as st
import pandas as pd
import matplotlib.pyplot as plt
...
# Sidebar to change different visualizations
tab = st.sidebar.radio("Select a Visualization", ['Health Statistics', 'Causes of Deaths'])
...
# Visualization Tab
if tab == 'Health Statistics':
    st.title('Health Statistics Dashboard')

    # Visualization 1 - Main Dashboard, Total deaths over time
    st.subheader('Total Deaths Over Time')

    year_range = st.slider('Select Year Range', min_value=int(death_data['Year'].min()), max_value=int(death_data['Year'].max()), value=(1990, 2019), key='slider_1')
    total_deaths = death_data.groupby('Year')['Total Deaths'].sum().loc[1990:2019].sum()
    st.write(f'Total Deaths from 1990 to 2019: {total_deaths}')
    
    year_data = death_data[(death_data['Year'] >= year_range[0]) & (death_data['Year'] <= year_range[1])]
    total_deaths_selected_year = year_data['Total Deaths'].sum()

    fig, ax = plt.subplots()
    ax.plot(year_data.groupby('Year')['Total Deaths'].sum())
    ax.set_xlabel('Year')
    ax.set_ylabel('Total Deaths')
...
# Visualization 7 - Communicable death
elif tab == 'Causes of Deaths':
    st.title('Communicable Diseases')

    comm_disease_column = ['Malaria', 'Tuberculosis', 'HIV/AIDS', 'Lower Respiratory Infections', 'Diarrheal Diseases',
                            'Neonatal Disorders', 'Environmental Heat and Cold Exposure', 'Conflict and Terrorism',
                            'Road Injuries', 'Acute Hepatitis']

    selected_comm_diseases = st.multiselect('Select Communicable Diseases', comm_disease_column, default=comm_disease_column)
    comm_disease_data = death_data[selected_comm_diseases] # Filter for selected communicable disease
    
    fig_barComm, ax_barComm = plt.subplots()
    comm_disease_data.sum().sort_values().plot(kind='barh', ax=ax_barComm)
    ax_barComm.set_xlabel('Total Deaths')
    ax_barComm.set_ylabel('Diseases')
    ax_barComm.set_title('Total Deaths for Selected Communicable Diseases')


    # Visualization 8 - Communicable Diseases Over Time
    st.title('Trend of Communicable Diseases Over Time')

    timeline_year_range = st.slider('Select Year Range', min_value=int(death_data['Year'].min()), max_value=int(death_data['Year'].max()), value=(1990, 2019), key='timeline_year_range')
    comm_disease_data_range = death_data[(death_data['Year'] >= timeline_year_range[0]) & (death_data['Year'] <= timeline_year_range[1])] 
    total_deaths_per_year = comm_disease_data_range.groupby('Year')[selected_comm_diseases].sum().reset_index()

    fig_lineComm, ax_lineComm = plt.subplots()
    for disease in selected_comm_diseases:
        ax_lineComm.plot(total_deaths_per_year['Year'], total_deaths_per_year[disease], label=disease, marker='o')

    ax_lineComm.set_xlabel('Year')
    ax_lineComm.set_ylabel('Total Deaths')
    ax_lineComm.legend()
    ax_lineComm.set_title('Trend of Communicable Diseases Over Time')
        
    for disease in selected_comm_diseases:
        total_death = comm_disease_data[disease].sum()
        st.write(f'Total Deaths for {disease}: {total_death}')
               
        
    col1_comm, col2_comm = st.columns(2)
    with col1_comm:
        st.pyplot(fig_barComm)
    with col2_comm:
        st.pyplot(fig_lineComm)
    
# Visualization 9 - Chronic Disease
    st.title('Chronic Diseases')

    chro_disease_column = ['Alzheimers Disease and Other Dementias','Parkinsons Disease','Nutritional Deficiencies','Cardiovascular Diseases','Neoplasms',
                           'Diabetes Mellitus','Chronic Kidney Disease','Cirrhosis and Other Chronic Liver Diseases'
                           ,'Digestive Diseases','Chronic Respiratory Diseases','Poisonings','Protein-Energy Malnutrition','Self-harm']

    selected_chro_diseases = st.multiselect('Select Chronic Diseases', chro_disease_column, default=chro_disease_column)
    chro_disease_data = death_data[selected_chro_diseases]

    fig_barChro, ax_barChro = plt.subplots()
    chro_disease_data.sum().sort_values().plot(kind='barh', ax=ax_barChro)
    ax_barChro.set_xlabel('Total Deaths')
    ax_barChro.set_ylabel('Diseases')
    ax_barChro.set_title('Total Deaths for Selected Chronic Diseases - Bar Chart')


    # Visualization 10 - Chronic Diseases Over Time
    st.title('Trend of Chronic Diseases Over Time')

    chronic_timeline_year_range = st.slider('Select Year Range', min_value=int(death_data['Year'].min()), max_value=int(death_data['Year'].max()), value=(1990, 2019), key='chronic_timeline_year_range')
    chro_disease_data_range = death_data[(death_data['Year'] >= chronic_timeline_year_range[0]) & (death_data['Year'] <= chronic_timeline_year_range[1])]
    total_deaths_per_year_chronic = chro_disease_data_range.groupby('Year')[selected_chro_diseases].sum().reset_index()

    fig_lineChro, ax_lineChro = plt.subplots()
    for disease in selected_chro_diseases:
        ax_lineChro.plot(total_deaths_per_year_chronic['Year'], total_deaths_per_year_chronic[disease], label=disease, marker='o')

    ax_lineChro.set_xlabel('Year')
    ax_lineChro.set_ylabel('Total Deaths')
    ax_lineChro.legend()
    ax_lineChro.set_title('Trend of Chronic Diseases Over Time - Line Chart')

    col1_chro, col2_chro = st.columns(2)
    with col1_chro:
        st.pyplot(fig_barChro)
    with col2_chro:
        st.pyplot(fig_lineChro)

    for disease in selected_chro_diseases:
        total_death = chro_disease_data[disease].sum()
        st.write(f'Total Deaths for {disease}: {total_death}')
    
    # Visualization 11 - External Disease
    st.title('External Causes')

    ext_disease_column = ['Drowning','Interpersonal Violence','Maternal Disorders','Drug Use Disorders','Alcohol Use Disorders',
                           'Exposure to Forces of Nature','Fire, Heat, and Hot Substances']

    selected_ext_diseases = st.multiselect('Select External Causes', ext_disease_column, default=ext_disease_column)

    ext_disease_data = death_data[selected_ext_diseases]
    fig_barExt, ax_barExt = plt.subplots()
    ext_disease_data.sum().sort_values().plot(kind='barh', ax=ax_barExt)
    ax_barExt.set_xlabel('Total Deaths')
    ax_barExt.set_ylabel('Causes')
    ax_barExt.set_title('Total Deaths for Selected External Causes - Bar Chart')

    # Visualization 12 - External Causes Over Time
    st.title('Trend of External Causes Over Time')

    external_timeline_year_range = st.slider('Select Year Range', min_value=int(death_data['Year'].min()), max_value=int(death_data['Year'].max()), value=(1990, 2019), key='external_timeline_year_range')
    ext_disease_data_range = death_data[(death_data['Year'] >= external_timeline_year_range[0]) & (death_data['Year'] <= external_timeline_year_range[1])]
    total_deaths_per_year_external = ext_disease_data_range.groupby('Year')[selected_ext_diseases].sum().reset_index()

    fig_lineExt, ax_lineExt = plt.subplots()
    for disease in selected_ext_diseases:
        ax_lineExt.plot(total_deaths_per_year_external['Year'], total_deaths_per_year_external[disease], label=disease, marker='o')

    ax_lineExt.set_xlabel('Year')
    ax_lineExt.set_ylabel('Total Deaths')
    ax_lineExt.legend()
    ax_lineExt.set_title('Trend of External Causes Over Time - Line Chart')

    col1_ext, col2_ext = st.columns(2)
    with col1_ext:
        st.pyplot(fig_barExt)
    with col2_ext:
        st.pyplot(fig_lineExt)

    for disease in selected_ext_diseases:
        total_death = ext_disease_data[disease].sum()
        st.write(f'Total Deaths for {disease}: {total_death}')
...
Country/Territory"] == country) & 
    ...
    # Filter data based on selected country and year
    filtered_data = death_data[(death_data["Country/Territory"] == country) & (death_data["Year"] == year_range)]
    ...
    if len(filtered_data) == 0:
        ...
        if len(filtered_data) == 1:
            ...
            knn_accuracy = knn_classifier.score(X, y)
            ...
            st.write(f"Predicted presence of {disease} using KNN: {knn_prediction}")
            st.write("Accuracy (KNN):", knn_accuracy)
            
            st.write("Naive Bayes Model:")
            st.write(f"Predicted presence of {disease} using Naive Bayes: {nb_prediction}")
            st.write("Accuracy (Naive Bayes):", nb_accuracy)
            
            st.write("Decision Tree Model:")
            st.write(f"Predicted presence of {disease} using Decision Tree: {dt_prediction}")
            st.write("Accuracy (Decision Tree):", dt_accuracy)
    ...
    if len(filtered_data) == 0:
        ...
        if len(filtered_data) == 1:
            ...
            st.write("K-Nearest Neighbors Model:")
            st.write(f"Predicted presence of {disease} using K-Nearest Neighbors: {knn_prediction}")
            st.write("Accuracy (K-Nearest Neighbors):", knn_accuracy)
    ...
    st.title("Clustering Model for Disease Patterns")

    # Inputs
    n_clusters = st.slider("Select Number of Clusters", min_value=2, max_value=10, value=3)
    features = st.multiselect("Select Features for Clustering", cause_of_deaths)

    # Preprocess data
    X = death_data[features]
    ...
    plt.title('Clustering of Disease Patterns')
    plt.colorbar(label='Cluster')
    st.pyplot()

    # Evaluate clustering (if ground truth available)
    if 'Ground Truth' in death_data.columns:
        ground_truth = death_data['Ground Truth']
